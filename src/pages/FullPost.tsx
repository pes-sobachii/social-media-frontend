...
const FullPost: React.FC<{ auth: IAuthUser }> = ({ auth }) => {
	...
	const {
		...
		isLoading,
		isError,
	}: { data: IFullPost | undefined; isLoading: boolean; isError: boolean } =
		useGetPost(id)
	...
	return (
		...
		>
			...
			{data.image && (
				<Box textAlign={'center'}>
					<img
						...
						style={{ width: 'auto', maxHeight: '400px', margin: '0 auto' }}
						alt='poster'
					/>
				</Box>
			)}
			...
			<Divider />
			{auth._id ? (
				<CommentSection data={data.comments} id={data._id} />
			) : (
				<Box sx={{ py: 5, textAlign: 'center' }}>
					<Typography fontSize={20}>Log in to comment</Typography>
				</Box>
			)}
		</Box>
	)
