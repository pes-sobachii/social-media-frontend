	...
	return (
		...
		>
			...
			{users?.length === 0 ? (
				...
				>
					<Puff width={'100px'} height={'100px'} color={'lightblue'} />
				</Box>
			) : (
				<List sx={{ width: '100%', px: { xs: 1, sm: 5 } }}>
					{users?.map((user: IUnauthUser) => {
						const follows = user.followers.some(
								...
								<ListItemText
									...
									primary={
										<Link to={`/user/${user._id}`}>
											<Typography sx={{ fontSize: { xs: 14, sm: 26 } }}>
												{user.surname}
											</Typography>
										</Link>
									}
								...
								>
									{auth._id !== user._id &&
										(follows ? (
											...
											<Button variant={'contained'} disabled={isFollowing}>
												Follow
											</Button>
										))}
								</ListItemButton>
